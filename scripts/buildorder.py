...
PACKAGES_NOT_BUILT = frozenset(('wasi-libc', 'libdispatch', 'zig'))
...
def generate_full_buildorder(pkgs_map):
    ...
    for name, pkg in pkgs_map.items():
        # A package and all of its subpackages are generated by a single
        # build, so ignore dependencies on its own subpackages when computing
        # the build order. This avoids artificial parent<->subpackage cycles
        # (e.g. a package depending on its own -data subpackage and vice versa).
        subpkg_names = {subpkg.name for subpkg in pkg.subpkgs}
        remaining_deps[name] = set(pkg.deps) - subpkg_names
    ...
    if set(pkgs_map.values()) != set(build_order):
        # Genuine dependency cycles between unrelated packages can still occur
        # (e.g. libmp3lame <-> libmpg123 <-> pulseaudio <-> libsndfile). Since
        # build-package.sh resolves missing dependencies on its own, emit a
        # warning and append the remaining packages instead of failing.
        print("WARNING: Cyclic dependencies found, appending remaining packages:", file=sys.stderr)
        remaining_pkgs = sorted(
            {pkg for name, pkg in pkgs_map.items() if pkg not in build_order},
            key=lambda p: p.name,
        )
        build_order += remaining_pkgs
