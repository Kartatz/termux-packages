...
termux_step_pre_configure() {
	...
	# shellcheck disable=SC2153
	{
		...
		echo "MANPATH_DEFAULT=\"$TERMUX_PREFIX/share/man\""
		echo "MANDIR=\"$TERMUX_PREFIX/share/man\""
		echo "BINM_PAGER=\"pager\""
		echo "LN=\"ln -s -f\""
		echo "PREFIX=\"$TERMUX_PREFIX\""
		echo "CC=\"$CC\""
		echo "CFLAGS=\"$CFLAGS -std=c99 -DNULL=0 $CPPFLAGS\""
		echo "LDFLAGS=\"$LDFLAGS\""
		for HAVING in 'HAVE_STRLCAT' 'HAVE_STRLCPY' 'HAVE_SYS_ENDIAN' 'HAVE_ENDIAN' 'HAVE_NTOHL' 'HAVE_NANOSLEEP' 'HAVE_O_DIRECTORY' 'HAVE_ISBLANK' 'HAVE_STRPTIME'; do
			echo "$HAVING=1"
		done
	} > configure.local
}
